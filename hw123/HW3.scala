...
object HW3 {
  ...
  def W(g: Map[Variable, SimpleType], expr: LambdaExpression): (SimpleType => SimpleType, SimpleType) = {
    expr match {
      ...
      case LetIn(v, arg, e) => {
        ...
        val xqized = s1(t1) //hole
        val (s2, t2) = W(s1gx + (v -> xqized), e)
        ((x: SimpleType) => s2(s1(x)), t2)
      }

      case v@Variable(_, _) if gvarmap.contains(v) => (a => a, gvarmap(v))
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val vars = getFreeVars(Set.empty, expr)
      val (sbst, t) = W(vars.map(v => v -> newVar).toMap, expr)
      println(sbst(t))
      for (v <- vars) {
        println(rmap(v) ++ " : " ++ sbst(gvarmap(v)).toString)
      }
    } catch {
